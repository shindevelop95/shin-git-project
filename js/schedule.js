(function ($) {
  ...
  DayScheduleSelector.prototype.renderRows = function () {
    ...
    $.each(generateDates(start, end, interval), function (i, d) {
      ...
      $el.append('<tr><td class="time-label' + j++ + '">' + hmmAmPm(d) + '</td>' + daysInARow + '</tr>');
    });
  };
  ...
  DayScheduleSelector.prototype.select = function ($slot) { $slot.attr('id', 'selected'); }
  DayScheduleSelector.prototype.deselect = function ($slot) { $slot.removeAttr('id'); }

  function isSlotSelected($slot) { return $slot.is('[id]'); }
      ...
      if (!plugin.isSelecting()) {  // if we are not in selecting mode
        ...
        if (day == plugin.$selectingStart.data('day')) {  // if clicking on the same day column
          ...
          plugin.$el.find('.time-slot[data-day="' + day + '"]').filter('[data-selecting]')
            .attr('id', 'selected').removeAttr('data-selecting');
